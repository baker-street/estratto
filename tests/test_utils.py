...
def test__write_and_op_on_tmp():
    ...
    def readtmp(fname):
        with open(fname) as fp:
            return fp.read()
    assert(smpldata ==
           utils.write_and_op_on_tmp(smpldata, readtmp, suffix='.txt'))
...
def test__TZ_PAT_3():
    for offset in NA_TIME_ZONE_ABRVS.iterkeys():
        tdstr1 = TIME_STRING1 + ' ' + offset
        tdstr2 = TIME_STRING2 + ' ' + offset
...
def test__tz_abv_2_offset__test1():
    for abrv, offset in NA_TIME_ZONE_ABRVS.iteritems():
...
def test__normize_datetime_tmzone__with_offset_and_abv_cached_res():
    for _in, _out in NA_TIME_ZONE_CONVER_CACHED_RESs.iteritems():
