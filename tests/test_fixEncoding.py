...
def test_insane_unicode1():
    res = insane_unicode(TEST_STRING_1)
    assert(res ==
           u"If numbers aren't beautiful, I don't know what is. –Paul Erdős")
    assert(isinstance(res, unicode))
...
def test_auto_unicode_dang_it1():
    res = auto_unicode_dang_it(TEST_STRING_1)
    assert(res ==
...
def test_auto_unicode_dang_it2():
    res = auto_unicode_dang_it(TEST_STRING_2)
    assert(res ==
           u'"mas"')
    assert(isinstance(res, unicode))
...
def test_auto_unicode_dang_it3():
    res = auto_unicode_dang_it(TEST_STRING_3)
    assert(res ==
           u'"mas"')
    assert(isinstance(res, unicode))
...
def test_auto_unicode_dang_it4():
    res = auto_unicode_dang_it(TEST_STRING_4)
    assert(res ==
           u'"EUR"')
    assert(isinstance(res, unicode))
